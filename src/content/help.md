...
- **API 키 기억하기**를 체크하면 `localStorage`에 저장되어 다음 방문 시 자동 복원됩니다.
...
| API 키 기억하기 | 체크 시 `localStorage`에 저장, 체크 해제 시 즉시 삭제 |
| 저장됨 뱃지 | 키가 `localStorage`에 저장된 상태일 때 표시 |
